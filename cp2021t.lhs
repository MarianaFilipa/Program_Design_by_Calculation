...
\textbf{Alínea 1:}
Desenvolvendo a propriedade indicada ($out . in = id$):
\begin{eqnarray*}
\start
    |out . in = id|
%
\just\equiv{ def de in }
%
        |out . [X, num_ops] = id|
%
\just\equiv{ lei 20 e lei 17: k = id }
%
        |lcbr(
        out . X = id . i1
    )(
...
%
\just\equiv{ def num\_ops, lei 20 e lei 17 }
%
        |lcbr(
        out . X = id . i1
    )(
...
%
\just\equiv{ def ops, lei 20 e lei 17 }
%
        |lcbr(
        out . X = id . i1
        .&&&. out . N = id . i2 . i1
...
%
        ...
        .&&&. (out . uncurry(Un)) (a,b) = (i2 . i2 . i2) (a,b)
    )|
%
\qed
\end{eqnarray*}
...
\xymatrix@@C=2cm@@R=1cm{
    ExpAr A
        \ar[r]^(0.25){outList}\ar[d]_{|eval_exp|}
&
...
\\
    B
&
    |1 + (A + (BinOp >< (B >< B)) + (UnOp >< B))|
        \ar[l]_(0.65){|g_eval_exp a|}
}
\end{equation*}

\begin{code}
g_eval_exp a = either (const a) (either id g2) where
          g2 = either g3 g4
          g3(op,(b,c)) | op == Sum = b + c
                       | op == Product = b * c
          g4 (op,b) | op == Negate = negate b
                    | op == E = expd b
\end{code}
...
\\
    |ExpAr A| 
        \ar[u]^{|[(clean)]|}\ar[r]^(0.25){clean}
&
    |1 + (A + (BinOp >< (ExpAr A >< ExpAr A)) + (UnOp >< ExpAr A))|
        \ar[u]_{|id + (id + (id >< ([(clean)] >< [(clean)])) + (id >< [(clean)]))|}
}
\end{equation*}
...
clean (Bin op a b) | op == Product && (a == (N 0) || b == (N 0)) = Right(Left 0)
                   | otherwise = Right(Right(Left(op,(a,b))))
clean (Un a b) | a == E && b == (N 0) = Right(Left 1)
               | otherwise = Right(Right(Right((a,b))))
\end{code}
...
\xymatrix@@C=2cm@@R=1,5cm{
    ExpAr A
        \ar[r]^(0.20){outList}\ar[d]_{|cata (gopt a)|}
&
    |1 + (A + (BinOp >< (ExpAr A >< ExpAr A)) + (UnOp >< ExpAr A))|
        \ar[d]^{|id + (id + (id >< (cata (gopt a) >< cata (gopt a))) + (id >< cata (gopt a)))|}
\\
    B
&
    |1 + (A + (BinOp >< (B >< B)) + (UnOp >< B))|
...
\begin{code}
gopt a = either (const a) (either id g2) where
    g2 = either g3 g4
    g3(op,(b,c)) | op == Sum = b + c
                 | op == Product = b*c
    g4 (op,b) | op == Negate = negate b
...
\textbf{Alínea 4:}
\\
Antes de começar a definir a função analisamos a função \emph{sd}. Constatamos que a função sd\_gen terá que ser um par em que o segundo elemento será a derivada da expressão, assim:

Seja $C = ExpAr A$ e $D = ExpAr B$ \\
\vspace{0.3cm}
\begin{equation*}
\xymatrix@@C=2cm@@R=1.5cm{
    C
        \ar[d]_{|cata sd_gen|}\ar[r]^(0.3){|outList|}
&
...
sd_gen = either (split (const X) (N . const 1)) g1 where
    g1 = either (split (N) (N . const 0)) g2
    g2 = either g3 g4
    g3(op, ((a,b),(c,d)) ) | op == Sum = (Bin op a c ,Bin op b d)
                           | op == Product = ( Bin op a c , Bin Sum (Bin op a d) (Bin op b c))
    g4 (op,(a,b)) | op == Negate = (Un Negate a, Un Negate b)
                  | op == E = ((Un op a), (Bin Product (Un E a) (b) ))
\end{code}
...
À semelhança do exercício anterior, observando a definição de ad reparamos que ad\_gen será
...
\xymatrix@@C=2cm@@R=1.5cm{
    ExpAr A
        \ar[d]_{|cata ad_gen|}\ar[r]^(0.25){|outList|}
&
    |1 + (A + (BinOp >< (ExpAr A >< ExpAr A)) + (UnOp >< ExpAr A))|
...
\\
    |ExpAr A >< B|
&
    |1 + (A + (BinOp >< ((ExpAr A >< B) >< (ExpAr A >< B)) + (UnOp >< (ExpAr A >< B)))|
        \ar[l]_(0.75){|ad_gen|}
}
\end{equation*}
\vspace{0.3cm}
...
ad_gen v = either (split (const v) (const 1)) g1 where
    ...
    g2 = either g3 g4
    g3(op, ((a,b),(c,d)) ) | op == Sum = (a + c ,b + d)
                           | op == Product = ( a * c , (a*d) + (b*c))
    g4 (op,(a,b)) | op == Negate = (negate a, negate b)
                  | op == E = (expd a, (expd a * b ))
\end{code}
...
\begin{eqnarray*}
    C_0 = 1 
\end{eqnarray*}
Desenvolvendo a fórmula para n+1($C_n_+_1$) de modo a que esta tenha a chamada recursiva da mesma, obtemos:
\begin{eqnarray*}
    C_n_+_1 =&& \frac{(2n+2)!}{(n+2)! (n+1)! } = \frac{(2n+2) (2n+1)  (2n!)} {(n+2) (n+1) (n+1)! n!} \\ \\
    =&& \frac{ (2n+2) (2n+1)} {(n+2) (n+1)} C_n = \frac{ 2(n+1) (2n+1)} {(n+2) (n+1)} C_n \\ \\
    =&& \frac{4n+2}{n+2} C_n  \\
\end{eqnarray*}
Se definirmos $f x = 4n+2$ e $g x = n+2$ , teremos $c n$ em recursividade múltipla com $f n$ e $g n$ e obtemos no total três funções nessa mesma situação(recursividade múltipla):
\begin{spec}
c 0 = 1
c (n+1) = (f n * c n) / g n
...
Com base nas funções definidas em cima e segundo a regra de algibeira descrita na página \ref{pg:regra} deste enunciado, definimos:

\begin{code}
loop (c, f, g) = (div (f*c) g, f + 4, g + 1)
inic = (1, 2, 2)
prj (c, f, g) = c
\end{code}
por forma a que
...
Dado o tipo da função \emph{calcLine}, sabemos que deverá devolver uma função e, portanto, devemos pensar numa abordagem virada para a exponenciação:
...
Como a função \emph{calcLine} pode ser implementada como um catamorfismo de listas, pois a NPoint = [Q], e como já conhecemos cataList, precisamos de determinar a função h (função gene do catamorfismo).
\vspace{0.3cm}
\begin{eqnarray*}
\xymatrix@@C=2cm@@R=1,5cm{
    NPoint
        \ar[r]^{outList}\ar[d]_{|cata h|} 
&
    1+Q \times Q^*
        \ar[d]^{|id+id >< (cata h)|} 
\\
...
Observando o diagrama anterior, podemos concluir que a função h será do tipo:
    ...
    \begin{spec}
       h: 1 + Q >< (NPoint -> OverTime NPoint) -> (NPoint -> OverTime NPoint)
    \end{spec}

Considerando a função \emph{calcLine} fornecida no Anexo C:
...
\begin{itemize}
\item Calcular o Overtime Q entre d e a cabeça do NPoint (linear1d d x);
\item Transformar esse resultado numa lista de apenas um elemento [OverTime Q](singl . linear1d d x). Temos portanto : ([OverTime Q], Overtime NPoint)
\item Colocar \emph{Overtime} em evidência: OverTime ([Q], NPoint) (sequenceA [singl . linear1d d x, f xs])
\item Concatenar [Q] com NPoint : Overtime NPoint
\end{itemize}
...
Continuando o raciocínio, h2 trabalha com o caso em que a lista de NPoint não é vazia, ou seja NPoint tem uma cabeça e uma cauda. Seguindo o algoritmo já abordado e sabendo que terá de devolver uma função:
    ...
    \begin{spec}
    h2 :: Q >< (NPoint -> OverTime NPoint) -> (NPoint -> OverTime NPoint)
    h2 ((x,t),l) | l == [] = nil
                 | otherwise = concat . (sequenceA [singl . (linear1d a (head c)), b (tail c)])
    \end{spec}
...
Para esta segunda alínea, começamos por analisar o algoritmo dado no \emph{Anexo C} para determinar qual seria a melhor abordagem para desenvolver este hilomorfismo.
...
Começamos por observar como seria feita a recursividade neste algoritmo. Podemos concluir que este algoritmo segue uma estratégia do tipo \emph{divide\&conquer}. De forma a facilitar a compreensão/análise, optamos por analisar o algoritmo com um exemplo de um possivel NPoint.
Seja um NPoint [[1,2],[3,4],[5,6]]

\[
\xymatrix@@C=0,4cm{
& & & {[[1,2],[3,4],[5,6]]}\ar[dll]_{init}\ar[drr]^{tail} & & &\\
& {[[1,2],[3,4]]}\ar[dl]_{init}\ar[dr]^{tail} & & & & {[[3,4],[5,6]]}\ar[dl]_{init}\ar[dr]^{tail} &\\
{[[1,2]]} & & {[[3,4]]} & & {[[3,4]]} & & {[[5,6]]}\\
\\
}
\]
este será o comportamento da parte \emph{divide} do algoritmo; de seguida, a parte \emph{conquer} do algoritmo\newline
\[
\xymatrix@@C=0,4cm{
\underline{[1,2]}\ar[dr] & & \underline{[3,4]}\ar[dl] & & \underline{[3,4]}\ar[dr] & & \underline{[5,6]}\ar[dl]\\
& {calcLine [1,2] [3,4]}\ar[drr] & & & & {calcLine [3,4] [5,6]}\ar[dll] &\\
& & & {resultado} & & &\\
\\
}
\]
...
\xymatrix{
& & & {[[1,2],[3,4],[5,6]]}\ar[dll]_{init}\ar[drr]^{tail} & & &\\
& \ar[dl]_{init}\ar[dr]^{tail} & & & & \ar[dl]_{init}\ar[dr]^{tail} &\\
\underline{[1,2]}\ar[dr] & & \underline{[3,4]}\ar[dl] & & \underline{[3,4]}\ar[dr] & & \underline{[5,6]}\ar[dl]\\
& \ar[drr] & & & & \ar[dll] &\\
& & & {resultado} & & &\\
\\
...
deCasteljau l = hyloAlgForm alg coalg l where
  coalg [n] = i1(n)
  coalg xs  = i2(init xs, tail xs)
  alg = either (const) (curry g)
  g ((a,b),c) = (calcLine (a c) (b c)) c
...
\begin{code}
outListNaoVazia [a] = i1 (a)
outListNaoVazia (a:t) = i2(a,t)
\end{code}

logo $|in = either singl cons|$

Assim, podemos concluir que o catamorfismo de uma lista não vazia será:
\begin{code}
cataListNaoVazia g = g . recList (cataListNaoVazia g) . outListNaoVazia
\end{code}

Sabendo que queremos definir o $|split avg length|$ , como um $|cata (either b q)|$ :

Seja avg e length: 
\begin{eqnarray*}
\start
\just\equiv{ Definição de avg e length }
  %
      \begin{lcbr}
          \begin{lcbr}
              |avg[a] = a|\\
      ...
      \end{lcbr}
  %
  \just\equiv{ singl a = [a], lei 1, cons (a,x) = (a:x) e succ x = x+1}
  %
      \begin{lcbr}
          \begin{lcbr}
              |avg (singl a) = id a|\\
              |avg (cons (a,x)) = frac(id a + length x * avg x)(succ (length x))|\\
          \end{lcbr}\\ \\
          \begin{lcbr}
      ...
      \end{lcbr}
  %
  \just\equiv{ lei 76, lei 79, add (a,b) = a+b e mul (a,b) = a*b e lei 7}
  %
      \begin{lcbr}
          \begin{lcbr}
              |avg (singl a) = id a|\\
              |avg (cons (a,x)) = frac (add (id a, (mul (split avg length) x ))) (succ (length (p2 (a,x))))|\\
          \end{lcbr}\\ \\
          \begin{lcbr}
              |length (singl a) = (const 1) a|\\
      ...
      \end{lcbr}
  %
  \just\equiv{ lei 72, lei 71 e lei 77}
 %
      \begin{lcbr}
          \begin{lcbr}
              |avg . singl = id|\\
              |avg . cons = uncurry (/) . split (add . (id >< (mul . (split avg length)))) (succ . length . p2 )|\\
          \end{lcbr}\\ \\
          \begin{lcbr}
      ...
      \end{lcbr}
  %
  \just\equiv{ lei 13, lei 14 e lei 7}
 %
      \begin{lcbr}
          \begin{lcbr}
              |avg . singl = id|\\
              |avg . cons = uncurry (/) . split (add . (id >< mul) . (id >< (split avg length))) (p2 . (id >< succ . p2) . (id >< split avg length))|\\
          \end{lcbr}\\ \\
          \begin{lcbr}
              |length . singl = const 1|\\
              |length . cons = (p2 . (id >< succ . p2) . (id >< split avg length))|\\
          \end{lcbr}
      \end{lcbr}
  %
    \just\equiv{ lei 9 }
  %
      \begin{lcbr}
          \begin{lcbr}
              |avg . singl = id|\\
              |avg . cons = uncurry (/) . (split (add . (id >< mul)) (p2 . (id >< succ . p2))) . (id >< (split avg length))|\\
          \end{lcbr}\\ \\
          \begin{lcbr}
              |length . singl = const 1|\\
              |length . cons = (p2 . (id >< succ . p2)) . (id >< split avg length)|\\
      ...
      \end{lcbr}
  %
      \just\equiv{ lei 27 e lei 20 }
  %
      \begin{lcbr}
              |avg . either singl cons = either id (uncurry (/) . (split (add . (id >< mul)) (p2 . (id >< succ . p2))) . (id >< (split avg length)))|\\
          \\
              |length . either singl cons = either (const 1) ((p2 . (id >< succ . p2)) . (id >< split avg length))|\\
      \end{lcbr}
  %
        \just\equiv{ lei 22 e lei 1 }
  %
      \begin{lcbr}
              |avg . either singl cons = either id (uncurry (/) . (split (add . (id >< mul)) (p2 . (id >< succ . p2)))) . (id + id >< (split avg length))|\\
           \\
              |length . either singl cons = either (const 1) ((p2 . (id >< succ . p2))) . (id + id >< split avg length))|\\
      \end{lcbr}
  %
          \just\equiv{ |F (split avg length) = id + id >< (split avg length)|, lei 52 }
          |split avg length = cata (split (either id (uncurry (/) . (split (add . (id >< mul)) (p2 . (id >< succ . p2))))) (either (const 1) ((p2 . (id >< succ . p2)))))|
  %
  %
          \just\equiv{ lei 28 e lei 13 }
          |(split avg length) = cata (either (split id (const 1)) (split (uncurry (/) . (split (add . (id >< mul)) (succ . p2 . p2))) (succ . p2 . p2)))|
  %
\qed
\end{eqnarray*}
...
avg_aux = cataListNaoVazia (either b q) where
  b = split id (const 1)
  q = split f g
  f = uncurry (/) . split (uncurry (+) . (id >< (uncurry (*)))) (succ . p2 . p2)
  g = succ . p2 . p2
...
\xymatrix@@C=2cm@@R=1,5cm{
    LTree A
        \ar[r]^(0.40){outLTree}\ar[d]_{|cataLTree gene|} 
&
    A+ (LTree A  \times LTree A)
        \ar[d]^{|id+ ( (cataLTree gene) >< (cataLTree gene) )|} 
\\
    |B >< C| 
& 
    |A + ((B >< C) >< (B >< C))|
